...
class SWBild
{
    ...
    public void horizontalSpiegeln()
    {
        ...
        for (int y = 0; y<_hoehe/2;y++)
        {
            for (int x = 0; x < _breite; x++)
            {
                short aWert = _bilddaten[y][x] ;
                short bWert = _bilddaten[_hoehe-1-y][x];
                _bilddaten[y][x] = bWert;
                _bilddaten[_hoehe-1-y][x] = aWert;
            }
        }
        zeichneBild();
    }
    ...
    {
        int[][] neuesBild = new int [_hoehe][_breite];
        ...
        for (int y = 0; y<_hoehe;y++)
        {
            for (int x = 0; x < _breite; x++)
            {
                ...
                for (int i = 0; i<3 ; i++)
                {
                    for (int j = 0;j<3;j++)
                    {
                        if (inBound(_hohe, _breite ,x-1+i,y-1+j)){
                        _bilddaten[y-1+j][x-1+i]+=wZahl;
                    }
                        
                    }
                }
                neuesBild[y][x] = wZahl/9;
            }
        }
        zeichneBild();
    }
    
    public boolean inBound(int yMax, int xMax, int x, int y){
